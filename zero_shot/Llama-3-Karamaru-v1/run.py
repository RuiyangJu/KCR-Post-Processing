...
if isinstance(model_eos_token_id, int):
    EOS_TOKEN_ID.append(model_eos_token_id)
elif isinstance(model_eos_token_id, list):
    EOS_TOKEN_ID.extend(model_eos_token_id)
...
EOS_TOKEN_ID = sorted(set(EOS_TOKEN_ID))
...
def correct_ocr_text(ocr_text: str) -> tuple[str, float]:
    ...
    token_ids = model_inputs["input_ids"].to(model.device)
